...
def note_on(output_port, note=60, velocity=64):
...
def note_off(output_port, note=60, velocity=64):
    if output_port is None:
...
def play_note(output_port, note=60, velocity=64, duration=1.0):
    ...
    try:
        note_on(output_port, note, velocity)
        
        # Hold for the specified duration
        time.sleep(duration)
        
        note_off(output_port, note, velocity)
    except Exception as e:
